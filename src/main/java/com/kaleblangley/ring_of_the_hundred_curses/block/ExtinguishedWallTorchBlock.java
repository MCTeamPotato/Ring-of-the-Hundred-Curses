...
public class ExtinguishedWallTorchBlock extends WallTorchBlock {
    ...
    public ExtinguishedWallTorchBlock(Properties pProperties) {
        super(pProperties, ParticleTypes.SMOKE);
    }

    @Override
    public void animateTick(BlockState pState, Level pLevel, BlockPos pPos, RandomSource pRandom) {
        // No particles for extinguished wall torch
    }
    ...
    public InteractionResult use(BlockState pState, Level pLevel, BlockPos pPos, Player pPlayer, InteractionHand pHand, BlockHitResult pHit) {
        ...
        if (stack.getItem() instanceof FlintAndSteelItem || stack.getItem() instanceof FireChargeItem) {
            if (!pLevel.isClientSide) {
                BlockState litState = Blocks.WALL_TORCH.defaultBlockState().setValue(WallTorchBlock.FACING, pState.getValue(WallTorchBlock.FACING));
                pLevel.setBlock(pPos, litState, 11);
